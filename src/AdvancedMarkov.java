public class AdvancedMarkov implements Player {
    int[][][] m = new int[3][3][3];
    int p = -1;
    int p2 = -1;
    ...
    private int getOpp() {
        ...
        int n = getSize(m[p2][p]);
        int rand = (int) (Math.random() * (n + 1));
        if (rand <= m[p2][p][RPS.ROCK]) return RPS.ROCK;
        else if (rand <= m[p2][p][RPS.ROCK] + m[p2][p] [RPS.PAPER]) return RPS.PAPER;
